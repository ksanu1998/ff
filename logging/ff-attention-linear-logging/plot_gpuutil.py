...
with open(fname) as csvfile:
	...
	for row in reader:
		if len(row) != 0:
			values.append(float(row[2].replace('%','').replace(' ','')))
		count += 1
...
plt.savefig(os.path.join(plot_directory, dataset, "gpu_util.png"), format='png', bbox_inches='tight', dpi=600)
