...
with open(fname) as csvfile:
	...
	for row in reader:
		if len(row) != 0:
			values.append(float(row[3].replace('%','').replace(' ','')))
		count += 1
...
axs.set_ylabel('Memory Utilization (%)', size=15)
'''
if dataset == "MNIST":
...
plt.savefig(os.path.join(plot_directory, dataset, "mem_util.png"), format='png', bbox_inches='tight', dpi=600)
